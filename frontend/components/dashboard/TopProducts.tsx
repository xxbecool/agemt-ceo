              ...
              {products?.map((product) => (
                <tr key={product.id} className="border-b border-slate-700/30 hover:bg-slate-700/20 transition-colors">
                  ...
                  <td className="px-4 py-3 text-right">
                    <span className="font-semibold text-white tabular-nums">{formatCurrency(product.revenue, { compact: true })}</span>
                  </td>
                  <td className="px-4 py-3 text-right">
                    <span className="text-slate-300 tabular-nums">{product.units.toLocaleString()}</span>
                  </td>
                  <td className="px-6 py-3 text-right">
                    <div className={cn("inline-flex items-center gap-1 text-sm font-semibold", product.growth >= 0 ? "text-emerald-400" : "text-red-400")}>
                      {product.growth >= 0 ? <TrendingUp className="w-3.5 h-3.5" /> : <TrendingDown className="w-3.5 h-3.5" />}
